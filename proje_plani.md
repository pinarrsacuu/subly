...
- 2026-09-08: **Büyük bir oturumda altyapı, üyelik, ödeme hazırlığı ve marka/landing page yeniden tasarımı tamamlandı.**
  ...
  - **Render Starter plan ($7/ay) yükseltmesi doğrulandı.** Başta "Plan update error: ... Plan requires payment information on file" hatası birkaç kez alınmıştı, ama Render dashboard'da servis "0.5c-512mb" (Starter) olarak göründüğü teyit edildi — sorun çözülmüş.
  ...
  - **Plan limitleri yeniden düzenlendi + IP bazlı kötüye kullanım koruması eklendi** (commit `3ec7a7a`). Kullanıcı, aynı IP'den farklı Google hesaplarıyla free limitin aşılabileceğini fark etti (3 video test etti, 4.'de limit hatası aldı, "aynı IP üzerinden birkaç mail'de bu çalışılabilir mi" diye sordu). Yeni plan yapısı: **Free** (3 video/ay, video başına 3 dk — eskiden 90 sn; async mimariye geçildiği için Cloudflare timeout riski kalktı, bu yüzden yükseltildi), **Pro** (149 TL/ay, 20 video/ay, 8 dk/video), **Premium** (349 TL/ay, 15 video/ay, 20 dk/video). `usage_tracker.py`'ye `usage_ip` tablosu eklendi — sadece **free plandaki** kullanıcılar için email sayacına ek olarak IP bazlı da sayılıyor (ödeme yapan kullanıcılar ortak ofis/wifi IP'sinde haksız yere engellenmesin diye pro/premium'da IP kontrolü yok). Bu %100 çözüm değil (VPN, mobil operatör NAT ile aşılabilir) ama kolay çoklu-hesap kaçamağını belirgin şekilde zorlaştırıyor. Ayrıca Premium'un 20 dakikaya çıkmasıyla Whisper API'nin 25MB dosya sınırına yaklaşma riski için ses çıkarma (`extract_audio`) mono + 64kbps'e düşürüldü (20 dk ses ~9.6MB, rahat pay bırakıyor).
  - **Bilinen kalite ödünü: çıktı videoları 720p'ye sabitleniyor.** Kullanıcı sordu: yükleme sırasında bir kısıtlama yok, insanlar orijinal çözünürlükte (1080p, 4K) yüklüyor — ama OOM'u önlemek için eklenen `SCALE_FILTER` (bkz. yukarıdaki `burn_captions.py` notu) sadece iç işlem için değil, **kullanıcının indirdiği son video için de** geçerli. Yani 4K yükleyen biri 720p indiriyor (720p ve altı videolar zaten `min(1280,...)` sayesinde etkilenmiyor, büyütme yapılmıyor). Kullanıcı bunu bilinçli bir ödün olarak kabul etti, şimdilik değiştirmiyoruz. **Plan:** Render'da Standard plana (2GB RAM, ~25$/ay) geçilince bu 720p sınırı kaldırılıp orijinal çözünürlük korunacak şekilde kod güncellenecek — kullanıcı bu yükseltmeyi ileride yapmayı düşünüyor ("büyüteceğiz 25 dolarlığa geçeceğiz muhtemelen").
  - **Sonraki adımlar:** (1) gerçek ~15-20 dakikalık videoyla Premium'un süre sınırını ve bellek dayanıklılığını ampirik olarak doğrula (gerekirse Render planını yükselt), (2) Render Standard'a geçilince 720p kısıtlamasını kaldır (orijinal çözünürlüğü koru), (3) iyzico başvurusu tamamlanınca ödeme entegrasyonunu kur, (4) ondan sonra pazarlama/lansman (Product Hunt, Indie Hackers, SEO) adımına dön.
